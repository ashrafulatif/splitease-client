...
import { MoreHorizontal, Edit, Trash2, Home, Users, Calendar, Banknote } from "lucide-react";
import {
  DropdownMenu,
  DropdownMenuContent,
  DropdownMenuItem,
      ...
      <div className="rounded-xl border bg-card shadow-sm overflow-hidden">
        <Table >
          <TableHeader className="bg-muted/50">
            <TableRow >
              <TableHead className="w-[280px] pl-6">House</TableHead>
              ...
              <TableRow>
                <TableCell colSpan={4} className="text-center h-48 text-muted-foreground">
                  <div className="flex flex-col items-center justify-center space-y-3">
                    ...
                    <span>No houses found. Click "Add House" to create one.</span>
                  </div>
                </TableCell>
              </TableRow>
            ) : (
              houses.map((house: any) => (
                <TableRow key={house.id || house._id} className="hover:bg-muted/50 transition-colors">
                  ...
                  <TableCell>
                    <div className="flex flex-wrap gap-2">
                       ...
                       <Badge variant="secondary" className="flex items-center gap-1 font-normal bg-purple-50 text-purple-700 dark:bg-purple-900/30 dark:text-purple-400">
                          <Calendar className="w-3 h-3" />
                          {house._count?.months || 0} Months
                       </Badge>
                       <Badge variant="secondary" className="flex items-center gap-1 font-normal bg-emerald-50 text-emerald-700 dark:bg-emerald-900/30 dark:text-emerald-400">
                  ...
                  <TableCell>
                     <div className="flex flex-col">
                       <span className="text-sm font-medium">{house.creator?.name || 'Unknown'}</span>
                       <span className="text-xs text-muted-foreground">{house.creator?.email}</span>
                     </div>
                  </TableCell>
                  ...
                  <TableCell className="text-right pr-6">
                    <DropdownMenu>
                      ...
                      <DropdownMenuContent align="end" className="w-[160px]">
                        <DropdownMenuItem onClick={() => onEdit(house)} className="cursor-pointer">
